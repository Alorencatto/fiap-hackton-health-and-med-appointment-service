...
def get_health_service(
    ...
    return HealthService(db, rabbitmq_host="localhost")
...
def get_appointment_publisher() -> (
    AppointmentUpdatePublisher
):  # TODO this should be an interface
    connection_params = pika.ConnectionParameters(
        host="localhost", heartbeat=120
    )
    return AppointmentUpdatePublisher(connection_params)



def get_appointment_service(
    db: Session = Depends(get_db),
    appointment_update_publisher: AppointmentUpdatePublisher = Depends(get_appointment_publisher),
) -> AppointmentService:
    appointment_repository = SQLAlchemyAppointmentRepository(db)
    return AppointmentService(
        appointment_repository,
        appointment_update_publisher
    )
